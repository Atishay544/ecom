...
export async function POST(req: NextRequest, { params }: PageProps) {
  ...
  if (!carrier_id) return NextResponse.json({ error: 'carrier_id is required' }, { status: 400 })

  // Load carrier config from DB
  ...
  if (!carrier) return NextResponse.json({ error: 'Carrier not found or inactive' }, { status: 404 })
  ...
  const { data: order } = await admin
    .from('orders')
    .select('id, total, user_id, status, metadata, shipping_address, order_items(quantity, unit_price, snapshot), created_at')
    .eq('id', id)
    .single()
